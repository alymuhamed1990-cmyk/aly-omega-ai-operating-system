...
async function search(task){const calls=[];for(const q of queries(task)){const e=encodeURIComponent(q);calls.push(['Bing',`https://www.bing.com/search?format=rss&q=${e}`]);calls.push(['DuckDuckGo',`https://html.duckduckgo.com/html/?q=${e}`]);calls.push(['Brave',`https://search.brave.com/search?q=${e}`]);calls.push(['Google via Jina',`https://r.jina.ai/https://www.google.com/search?q=${e}`]);}const responses=await Promise.all(calls.map(async([source,url])=>{const r=await get(url);return{source,status:r.status,ok:r.ok,raw:r.text,results:r.ok?parseAny(r.text,source):[]}}));const map=new Map();for(const p of responses)for(const x of p.results){const u=x.url.replace(/#.*$/,'');if(!map.has(u))map.set(u,{...x,url:u,domain:host(u),sources:[p.source]});else map.get(u).sources.push(p.source)}const tokens=task.toLowerCase().split(/[^a-z0-9]+/).filter(w=>w.length>3&&!['find','best','with','from','that','this','near','into','for','and','the','hotels','hotel'].includes(w));const ranked=[...map.values()].map(x=>{const text=(x.title+' '+x.snippet+' '+x.domain).toLowerCase();const score=tokens.reduce((n,t)=>n+(text.includes(t)?1:0),0);return{...x,score}}).filter(x=>tokens.length<2||x.score>0).sort((a,b)=>b.score-a.score);return {results:ranked.slice(0,15),diagnostics:responses.map(p=>({source:p.source,status:p.status,ok:p.ok,count:p.results.length,sample:p.results.slice(0,3),preview:p.source==='Google via Jina'?String(p.raw||'').slice(0,500):undefined}))}}
async function run(task,emit){await emit('INTAKE','Request understood',{activeAgents:1});await emit('SEARCH','Searching multiple live web sources',{activeAgents:3});const searchData=await search(task);const found=searchData.results;await emit('EVIDENCE',`${found.length} unique results collected`,{activeAgents:2,count:found.length,diagnostics:searchData.diagnostics});const verified=found.filter(x=>x.domain&&x.title).map(x=>({...x,verification:'Source URL and result metadata present'}));await emit('VERIFY',`${verified.length} results passed basic evidence checks`,{activeAgents:2,count:verified.length});const final=verified.slice(0,10).map((x,i)=>({...x,rank:i+1}));await emit('FINAL',`Released ${final.length} evidence-backed results`,{activeAgents:1,count:final.length,released:true});return{results:final,message:final.length?'Results were collected from live web search and deduplicated across sources.':'No usable results were returned.'}}
